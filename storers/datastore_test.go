...
func (d *DatastoreFactory) NewStorer(ctx context.Context) (accounts.Storer, error) {
...
func (d *DatastoreFactory) TeardownStorers() error {
	...
	for _, namespace := range d.namespaces {
		q := datastore.NewQuery(datastoreAccountKind).Namespace(namespace).KeysOnly()
		keys, err := d.client.GetAll(context.Background(), q, nil)
		if err != nil {
			log.Printf("Error cleaning up accounts in namespace %q: %s", namespace, err.Error())
			continue
		}
